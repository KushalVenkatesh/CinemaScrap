    ...
    for scraped_movie in scraped_movies:
        try:
            ...
            if imdb_movie['rating'] >= NOTIFY_ABOVE_RATING:
                ...
                times = scraped_movie['times']
                good_movies.append([title, rating, times])
        except:
            pass
    if good_movies:
...
def send_email(movies):
    ...
    for movie in movies:
        html += "<p><b>{title}</b> => {rating}</p>".format(title=movie[0], rating=movie[1])
        text += "\n{title} => {rating}".format(title=movie[0], rating=movie[1])
        if not movie[2]:
            html += "<p><i>Keine weiteren Vorstellungen heute.</i></p><br>"
        else:
            for i in movie[2]:
                html += "<i>{}</i><br>".format(i)
